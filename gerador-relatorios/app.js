...
function clearAll(){
  ...
  if(modo==='parcial'){dadosParcial=[];renderParcial();}
  else{dadosFechamento=[];renderFechamento();}
  document.getElementById('resultsSection').style.display='none';
}
...
function downloadModelo(){
  let h,ex;
  if(modo==='parcial'){h='Nome,Ranking,Perf_Parcial_%,Perf_Mensal_%,NV_MetaP,NV_MetaM,NV_Real,TM_Meta,TM_Real,OV_MetaP,OV_MetaM,OV_Real,POS_MetaP,POS_MetaM,POS_Real,MIX_MetaP,MIX_MetaM,MIX_Real';ex='Carlos Martins / RP,13,77,41,1.43,3,0,3000,2094.53,16666.67,35000,12567.16,5.71,12,6,60,125,64';}
  ...
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`modelo_${modo}.csv`;a.click();
}
...
function handleCSV(e){
  ...
  r.onload=ev=>{
    ...
    if(modo==='parcial'){
      dadosParcial=[];
      lines.forEach(l=>{const c=parseCSV(l);if(c[0]){const d=novoParcial({nome:c[0],ranking:c[1],perfParcial:c[2],perfMensal:c[3],nvMetaP:c[4],nvMetaM:c[5],nvReal:c[6],tmMeta:c[7],tmReal:c[8],ovMetaP:c[9],ovMetaM:c[10],ovReal:c[11],posMetaP:c[12],posMetaM:c[13],posReal:c[14],mixMetaP:c[15],mixMetaM:c[16],mixReal:c[17]});calcParcial(d);dadosParcial.push(d);}});
      renderParcial();
    }else{
      dadosFechamento=[];
      lines.forEach(l=>{const c=parseCSV(l);if(c[0]){const d=novoFechamento({nome:c[0],ranking:c[1],perfMensal:c[2],nvMeta:c[3],nvReal:c[4],tmMeta:c[5],tmReal:c[6],ovMeta:c[7],ovReal:c[8],posMeta:c[9],posReal:c[10],mixMeta:c[11],mixReal:c[12]});calcFechamento(d);dadosFechamento.push(d);}});
      renderFechamento();
    }
...
function gerarRelatorio(d){return modo==='parcial'?relParcial(d):relFechamento(d);}
