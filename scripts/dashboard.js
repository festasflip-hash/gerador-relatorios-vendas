function renderDashboard(){
  ...
  const dados=(fonte==='fechamento'?dadosFechamento:dadosParcial).filter(d=>d.nome&&d.nome.trim());
  ...
  const alta=perfs.filter(p=>p>=90)
  .length,media=perfs
  .filter(p=>p>=50&&p<90)
  .length,baixa=perfs
  .filter(p=>p<50)
  ...
  const semMeta=fonte==='fechamento'?dados.filter(d=>{
  ...
  wrap.innerHTML=`
    <div class="dash-grid-top">
      ...
      <div class="dash-card">
        ...
        <div class="dc-val" style="color:${colorHex(mediaEq)}">${mediaEq.toFixed(0)}%</div>
    ...
    <div class="dash-card" style="text-align:left;padding:16px 18px">
      ...
      <div style="display:flex;justify-content:space-between;margin-bottom:5px">
        ...
        <span style="font-weight:700;color:#1a3c6e;font-size:12px">R$ ${fmt(totalMeta)}</span>
      </div>
      <div style="display:flex;justify-content:space-between;margin-bottom:6px">
        <span style="font-size:11px;color:#888">Realizado</span>
        <span style="font-weight:700;color:${colorHex(pctEq)};font-size:12px">R$ ${fmt(totalVendido)}</span>
      </div>
      <div style="height:7px;background:#eee;border-radius:10px;overflow:hidden;margin-bottom:6px">
        <div style="height:7px;width:${Math.min(pctEq,100)}%;background:${colorHex(pctEq)};border-radius:10px"></div>
      </div>
    ...
    <div class="dash-section">
      ...
      <div class="atencao-cards">
        ...
        ${fonte==='fechamento'?`<div class="atencao-card ${semMeta.length>0?'red':'green'}">
        ...
        <div class="atencao-card ${piorInd.pctMedio<60?'red':piorInd.pctMedio<90?'orange':'green'}">
          <div class="ac-icon">🎯</div>
          <div class="ac-title">Gargalo</div>
          <div class="ac-val">${piorInd.pctMedio.toFixed(0)}%</div>
          <div class="ac-desc">${piorInd.label}</div>
        </div>
      ...
      <div class="dash-section">
        ...
        <div class="dist-bars">
          <div class="dist-row">
            ...
            <div class="dist-track">
              <div class="dist-fill" 
              style="width:${total>0?alta/total*100:0}%;background:linear-gradient(90deg,#27ae60,#2ecc71)">${alta>0?alta:''}</div>
            </div>
        ...
        ${semMeta.length>0?`
          <div style="margin-top:16px">
            ...
            <div class="zero-list">${semMeta.map(d=>`<div class="zero-item">
                <div class="zero-dot"></div>
                ${d.nome}</div>`).join('')}
            </div>
          </div>`:''
        }
          ...
          ${gargalos.map((g,i)=>`
            <div class="gargalo-item">
              ...
              <div class="gargalo-info">
                <div class="gi-name">${g.label} 
                  <span style="font-size:10px;color:#aaa">peso ${g.peso}pts</span>
                </div>
                <div class="gi-sub">Média de atingimento da equipe</div>
              </div>
              <div class="gargalo-pct" style="color:${colorHex(g.pctMedio)}">${g.pctMedio.toFixed(0)}%</div>
            </div>`).join('')
          }
        </div>
      </div>
    ...
    <div class="dash-section">
        ...
        ${topPerf.map((d,i)=>`
          <div class="top-item">
            ...
            <div class="ti-info">
              ...
              <div class="ti-sub">${fonte==='fechamento'?`${[f(d.nvReal)>=f(d.nvMeta),f(d.tmReal)>=f(d.tmMeta),f(d.ovReal)>=f(d.ovMeta),f(d.posReal)>=f(d.posMeta),f(d.mixReal)>=f(d.mixMeta)].filter(Boolean).length}/5 metas batidas · Obj. Vendas: R$ ${fmt(f(d.ovReal))}`:`Perf. parcial: ${f(d.perfParcial)||'—'}%`}</div>
            </div>
            <div class="ti-pct">${f(d.perfMensal)}%</div>
          </div>`).join('')
        }
      </div>
      ...
      <table class="rank-table">
        ...
        <tbody>${ranking.map((d,i)=>{
          ...
          <tr>
            <td>
              <span class="rt-medal ${med}">${i+1}</span>
            </td>
            <td>
              <strong>${d.nome}</strong>
              ${pct<mediaEq?`
                <span style="font-size:10px;color:#e67e22;margin-left:6px">↓ abaixo da média</span>`:''
              }
            </td>
            <td>
              <span class="rt-bar-bg">
                <span class="rt-bar-f" style="width:${Math.min(pct,100)}%;background:${colorHex(pct)}"></span>
              </span> 
              <span style="font-weight:700;color:${colorHex(pct)}">${pct}%</span> 
              <span style="font-size:10px;font-weight:600;margin-left:4px;padding:2px 6px;border-radius:10px;background:${pct>=90?'#eafaf1':pct>=50?'#fef9e7':'#fdf2f2'};color:${colorHex(pct)}">${lbl}</span>
            </td>
            ...
            <td style="text-align:center">
              <span style="font-weight:800;color:${colorHex(ovPct)};font-size:12px">${ovPct.toFixed(0)}%</span>
                ${ovFalta>0?`
                  <br>
                    <span style="font-size:10px;color:#e74c3c">− R$ ${fmt(ovFalta)}</span>`:`
                  <br>
                    <span style="font-size:10px;color:#27ae60">✓</span>
                `}
            </td>
          </tr>`;}).join('')}
        </tbody>
